...
public class AddAddress extends JFrame {
	...
	public AddAddress(int uid, SQL sqlo, MainFrame mainFrame) {
		...
		jpanel.add(city);
		jpanel.add(new JLabel("Province:"));
		jpanel.add(province);
	...
	class addrlistener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if (Name.trim().isEmpty() || Pnum.trim().isEmpty() || Streetaddr.trim().isEmpty() || City.trim().isEmpty()
					|| Province.trim().isEmpty() || Postalcode.trim().isEmpty())
				JOptionPane.showMessageDialog(null, "It is required to fill in every blank", "Error",
						JOptionPane.ERROR_MESSAGE);
			else {
				sqlcode = "insert into address values (" + addrid + "," + userid + ", \'" + Name + "\', \'" + Pnum
						+ "\', \'" + Province + "\', \'" + City + "\', \'" + Streetaddr + "\', \'" + Postalcode + "\')"; // Insert
																															// a
																															// new
																															// address
																															// of
				...
				JOptionPane.showMessageDialog(null, "You have successfully added a new address", "Success",
						JOptionPane.INFORMATION_MESSAGE, new ImageIcon(AddAddress.class.getResource("success.png")));
